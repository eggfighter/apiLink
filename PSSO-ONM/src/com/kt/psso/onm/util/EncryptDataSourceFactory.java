...
public class EncryptDataSourceFactory implements ObjectFactory {
	...
	protected static Log LOG = LogFactory.getLog(EncryptDataSourceFactory.class);
	...
	public Object getObjectInstance(Object obj, Name name, Context nameCtx,
		...
		for (int i = 0; i < ALL_PROPERTIES.length; i++) {
			...
			if (ra != null) {
				...
				System.out.println(i+"."+propertyName+ ":"+propertyValue);
	...
	public static DataSource createDataSource(Properties properties)
		...
		if (value != null) {
			dataSource.setUrl(decryptDBCPProperty(value)); // 암호화된 데이터를 복호화 시켜주는
															// 함수
		}
	 ...
	 * 
	 * @param encrypted	String 암호화 된 Config 값, 여기서는 kt의 암복화화 모듈을 사용해서 복호화했다.
	 * @return String	복호화 된 Config 값
	 */
	private static String decryptDBCPProperty(String encrypted) {
		KTFCryptoUtil cryptoUtil = new KTFCryptoUtil();
		String result = null;
		try {
			
			result = cryptoUtil.decryptData(encrypted);
