 ...
 * ---------------------------------------------------------Project<br>
 * Project : e-HRD<br>
 * System Name : cutil File Name : StringUtil.java<br>
 * Creater : keegun<br>
...
public class StringUtil {
	 ...
	 * @return String
	 */
	public static String trimToKor( String str ) throws Exception {
		String sTmp = "";

		if( str == null ) {
			sTmp = "";
		} else if( !"".equals( str ) && str.length() > 0 ) {
			sTmp = EncodeUtil.setKorLng( str.trim() );
		}
	...
	public static String replace2(String value, String replace1, String replace2)
	{
		...
		while((searchNo = value.indexOf(replace1, startNo)) > -1)
		{
			stringBuffer.append(value.substring(startNo, searchNo));
			stringBuffer.append(replace2);
			startNo = searchNo + replace1.length();
		}

		stringBuffer.append(value.substring(startNo));
	...
	try
	{
		...
		fileName = EncodeUtil.URLDecode(fileName, "euc-kr");
    ...
    public static boolean checkEnglishNumber(String value)
    {
    ...
    public static boolean checkEnglishWithNumber(String value)
    {
    	...
    	boolean englishExitCheck = false;
    ...
    public static boolean checkWeakId(String value)
    {
    	if (value == null) return false;
    	
    	ArrayList<String> weakIds = new ArrayList<String>();
    	weakIds.add("ktf");
    	weakIds.add("kt68");
    	weakIds.add("nt");
    	weakIds.add("users");
    	weakIds.add("user");
    	weakIds.add("terminal");
    	weakIds.add("system");
    	weakIds.add("sid");
    	weakIds.add("service");
    	weakIds.add("server");
    	weakIds.add("self");
    	weakIds.add("restricted");
    	weakIds.add("pseudo");
    	weakIds.add("proxy");
    	weakIds.add("owner");
    	weakIds.add("null");
    	...
    	weakIds.add("logon");
    	weakIds.add("local");
    	weakIds.add("internet");
    	weakIds.add("interactive");
    	weakIds.add("guests");
    	weakIds.add("guest");
    	weakIds.add("group");
    	weakIds.add("everyone");
    	weakIds.add("enterprise");
    	weakIds.add("domain");
    	weakIds.add("dialup");
    	weakIds.add("creator");
    	weakIds.add("conttollers");
    	weakIds.add("builtin");
    	weakIds.add("batch");
    	weakIds.add("authorty");
    	weakIds.add("authenticated");
    	weakIds.add("anonymous");
    	weakIds.add("administrator");
    	weakIds.add("administrators");
    	weakIds.add("psso");
    	weakIds.add("admin");

    	for(String weakId : weakIds)
    	{
    		if(value.contains(weakId))
    		{
    			return false;
    		}
    	}    	
    ...
    public static boolean checkPswd(String value)
    {
    	if (value == null) return false;
    	...
    	for(int i = 0; i < value.length(); i++)
    	{
    		if((value.toLowerCase()).charAt(i) >= 'a' && (value.toLowerCase()).charAt(i) <= 'z')
    		{
    			englishExitCheck = true;
    	...
    	boolean SpecialExitCheck = false;
    	
    	ArrayList<String> specialCh = new ArrayList<String>();
    	specialCh.add("~");
    	specialCh.add("!");
    	specialCh.add("@");
    	specialCh.add("#");
    	...
    	for(String specialChs: specialCh)
    	{
    		if(value.contains(specialChs))
    		{
    			SpecialExitCheck = true;

    			break;
    		}
    	}

    	if(!numberExitCheck || !englishExitCheck || !SpecialExitCheck)
    	{
    		return false;
    	}
	...
	public static boolean contain123Abc(String value) {
		String abcd = " ";
		int k = 0;
		for(int i = 0; i < value.length(); i++) {
			if (value.charAt(i) - abcd.charAt(k) == 1) {
				k++;
				if (k < 2) {
	...
	public static boolean conta111Aaa(String value) {
		char[] abcd = new char[3];
		int k = 0;
		for(int i = 0; i < value.length(); i++) {
	...
	public static boolean checkEmailFormat(String email) {
		String regex = "^([a-z]+)([a-z0-9-_\\.]{1,100})([a-z0-9]+)@([a-z0-9]+)([a-z0-9-.]*)([a-z0-9]+)\\.([a-z]{2,6})$";
	...
	public static String markPhoneNumber(String str) {
		...
		return str.substring(0, 5) + "****" + str.substring(len - 2);
	}
	
	public static String markIp (String str) {
		if (str == null) return null;
		int len = str.length();
		if (len < 5) return str;
	...
	public static String markId(String str) {
		if (str == null) return null;
		int len = str.length();
		//if (len < 5) return str;
